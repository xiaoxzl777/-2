...
def score(findings: Iterable[Finding], mode: str = "hybrid") -> tuple[float | None, dict[str, float | None]]:
    """返回 (总分, 各维度分)。只统计通过证据校验的 finding。"""
    ...
    detail: dict[str, float | None] = {
        c: float(max(0, 100 - deducted[c])) if c in active else None for c in CATEGORIES}
