...
from tests.conftest import apply, fulltext_reply, judge_reply
...
def test_hybrid_report(client, auth_headers, resume_and_job, fake_llm, store):
    rid, jid = resume_and_job
    fake_llm.replies["_JudgeOut:有缓存性能优化经验"] = [judge_reply("hit", 1)]
    fake_llm.replies["_JudgeOut:熟悉 Kafka"] = [judge_reply("miss")]
    fake_llm.replies["_FulltextOut"] = [fulltext_reply((4, "miss", None))]
    ...
    # 简历单元只入库一次：再投一遍不会重新向量化整份简历
    embedded = len(store._embedder.embedded)
    _match(client, auth_headers, rid, jid)
    assert len(store._embedder.embedded) - embedded == 2                    # 只有两条要求的查询向量


def test_dict_only_touches_neither_the_model_nor_the_vector_store(client, auth_headers, resume_and_job, fake_llm, store):
    rid, jid = resume_and_job
